 ...
 */
function wrappedAmount(currencyAmount, chainId = ChainId.POLYGON) {
    if (currencyAmount instanceof TokenAmount)
        return currencyAmount;
    if (currencyAmount.currency === CAVAX[chainId])
        return new TokenAmount(WAVAX[chainId], currencyAmount.raw);
    invariant(false, 'CURRENCY');
}
function wrappedCurrency(currency, chainId = ChainId.POLYGON) {
 ...
 */
export class Trade {
    constructor(route, amount, tradeType, chainId = ChainId.POLYGON) {
     ...
     */
    static exactIn(route, amountIn, chainId = ChainId.POLYGON) {
     ...
     */
    static exactOut(route, amountOut, chainId = ChainId.POLYGON) {
     ...
     */
    minimumAmountOut(slippageTolerance, chainId = ChainId.POLYGON) {
     ...
     * @param slippageTolerance tolerance of unfavorable slippage from the execution price of this trade
     */
    maximumAmountIn(slippageTolerance, chainId = ChainId.POLYGON) {
        invariant(!slippageTolerance.lessThan(ZERO), 'SLIPPAGE_TOLERANCE');
    ...
    currentPairs = [], originalAmountIn = currencyAmountIn, bestTrades = []) {
        ...
        for (let i = 0; i < pairs.length; i++) {
            ...
            if (amountOut.token.equals(tokenOut)) {
                sortedInsert(bestTrades, new Trade(new Route([...currentPairs, pair], originalAmountIn.currency, currencyOut), originalAmountIn, TradeType.EXACT_INPUT, chainId), maxNumResults, tradeComparator);
            }
    ...
    currentPairs = [], originalAmountOut = currencyAmountOut, bestTrades = []) {
        ...
        for (let i = 0; i < pairs.length; i++) {
            ...
            if (amountIn.token.equals(tokenIn)) {
                sortedInsert(bestTrades, new Trade(new Route([pair, ...currentPairs], currencyIn, originalAmountOut.currency), originalAmountOut, TradeType.EXACT_OUTPUT, chainId), maxNumResults, tradeComparator);
            }
            else if (maxHops > 1 && pairs.length > 1) {
                const pairsExcludingThisPair = pairs.slice(0, i).concat(pairs.slice(i + 1, pairs.length));
